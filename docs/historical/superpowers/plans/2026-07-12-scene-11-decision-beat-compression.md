...
- [x] **Step 1: Run the presentation verification gate**
...
- [x] **Step 2: Run direct-route visual smoke**
...
- [x] **Step 3: Search for stale presentation-beat references**
...
- [x] **Step 5: Archive the completed plan and close the roadmap item**
...
- [x] Scene 11 has exactly `interrupt` and `approval` beats.
- [x] The approval surface still exposes both `Submit` and `Request revision`.
- [x] Canonical `submitted` and `cancelled` outcome data is unchanged.
- [x] `#scene/typed-human-boundary/cancel` fails closed instead of aliasing approval.
- [x] Approval advances directly to Scene 12 resume.
- [x] No live RPC, replay event, or Scene 12 behavior changed.
- [x] No stale cancel beat references remain in presentation navigation or requirements.
- [x] Presentation tests, typecheck, build, visual smoke, and review pass before archival.
